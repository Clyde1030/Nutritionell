# HTTPS for the backend API at api.<dns_zone_name>, terminated directly on the ALB.
# No CloudFront in front of this — the previous CloudFront config forwarded all
# cookies/headers/query strings with TTLs of 0, i.e. it cached nothing, so it added
# a hop without a caching benefit for this mutation-heavy API. Route53/zone lookup
# lives in dns.tf since Amplify's app domain needs the same zone.
...
locals {
  api_cert_validation_options = local.custom_domain_enabled ? {
...
resource "aws_route53_record" "api_cert_validation" {
  provider = aws.dns
  for_each = local.api_cert_validation_options
...
# No AAAA record: aws_lb.this (network.tf) doesn't set ip_address_type = "dualstack",
# so the ALB is IPv4-only and an AAAA alias would just resolve to nothing.

# --- Removed 2026-07-14: this file used to be edge_domain.tf, putting CloudFront
# in front of the ALB for a single apex custom domain (var.custom_domain_name,
# with www + other SANs). Replaced by the direct ALB HTTPS listener above (API)
# and amplify.tf (web_new) -- the distribution below cached nothing (TTLs were
# all 0), so it was only buying TLS termination + a domain alias, which the ALB
# listener above now does one hop closer to the app.
#
# locals {
