...
Every route requires an admin token. There is no UI for these — they are called
by hand with curl; see the "Admin approval (temporary)" section of
Application/backend/README.md for copy-pasteable commands.
...
async def revoke_user(user_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    if user.is_admin:
        message = (
            ...
            "Clear is_admin in the database to fully revoke."
        )
    elif already:
