...
Mirrors /api/analyze's contract deliberately: PUBLIC, with optional auth, and no
client-supplied profile_id. The frontend's USE_MOCK_ANALYZE flag points local
testing here, so if this drifted from the real route, flipping that flag would
quietly exercise a different auth behaviour than production.
"""
import io
...
async def mock_analyze(
    ...
    await image.read()
    result = MOCK_RESULT.model_copy(deep=True)
    if current_user is None:
        result.scored, result.auth_state = False, "anonymous"
    elif not current_user.has_access:
        result.scored, result.auth_state = False, "pending"
    return result
